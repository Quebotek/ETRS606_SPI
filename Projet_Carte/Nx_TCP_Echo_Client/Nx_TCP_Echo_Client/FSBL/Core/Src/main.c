...
#include <stdio.h> // <-- Indispensable pour printf et setvbuf
...
UART_HandleTypeDef huart1; // <-- On utilise bien huart1 comme configuré dans CubeMX
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // 1. Désactiver le buffer de printf (le texte s'affichera immédiatement)
  setvbuf(stdout, NULL, _IONBF, 0);

  // 2. Message de test matériel affiché via l'UART1
  printf("\r\n\r\n================================================\r\n");
  printf(">>> DEMARRAGE DU SYSTEME - STM32N6 (FSBL) <<<\r\n");
  printf("================================================\r\n");
  /* USER CODE END 2 */
...
// Redirection absolue de printf() vers USART1 pour le compilateur GCC (STM32CubeIDE)
#ifdef __GNUC__
int _write(int file, char *ptr, int len)
{
    // On envoie les caractères sur l'USART1
